...
function startBot(token, isMain = false) {
    ...
    S7.on('photo', async (msg) => {
        ...
        try {
            ...
            S7.sendMessage(chatId, "<b><tg-emoji emoji-id="
                4972341539033318152 "></tg-emoji> Error saving logo. Please try a different photo.</b>", {
                    parse_mode: "HTML"
                });
        }
    });
    ...
    SYLoVe('add', (msg, S7, chatId, userId) => {
        ...
        if (args.length < 3) {
            return S7.sendMessage(chatId,
                "<b><tg-emoji emoji-id="
                4972341539033318152 "></tg-emoji> Usage: /add category your message\nCategories: love, sad, god</b>", {
                    parse_mode: "HTML"
                }
            );
        }
    ...
    SYLoVe('cap', (msg, S7, chatId) => {
        ...
        if (!category) {
            return S7.sendMessage(chatId,
                "<b><tg-emoji emoji-id="
                4972341539033318152 "></tg-emoji> Usage: /cap category \nExample: /cap love</b>", {
                    parse_mode: "HTML"
                }
            );
        }
